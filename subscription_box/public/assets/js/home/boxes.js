...
function updateAuthUI() {
    ...
    const adminOnlyHiddenLinks = document.querySelectorAll('.navbar .nav-link[data-public-nav]');
    if (session) {
        if (authActionLink) {
            ...
            <div class="box-card h-100" onclick="openModal(${b.id})" style="cursor:pointer;">
                ...
                <div class="p-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <a href="${sportBoxRoute('customize')}?box=${b.id}" class="btn btn-primary btn-sm px-3" onclick="event.stopPropagation()"><i class="bi bi-pencil-square me-1"></i>Customize</a>
                    </div>
                </div>
            </div>`;
...
function openModal(id) {
    ...
    const customizeLink = document.getElementById('modalCustomizeLink');
    if (customizeLink) customizeLink.href = `${sportBoxRoute('customize')}?box=${selectedBox.id}`;
